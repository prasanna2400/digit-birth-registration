...
@Component
public class BirthRegistrationQueryBuilder {
    ...
    private static final String ADDRESS_SELECT_QUERY = " add.id as aid, add.tenantid as atenantid, add.doorno as adoorno, add.latitude as alatitude, add.longitude as alongitude,  add.addressnumber as aaddressnumber,  add.addressline1 as aaddressline1, add.addressline2 as aaddressline2, add.landmark as alandmark, add.street as astreet, add.city as acity,  add.pincode as apincode, add.detail as adetail, add.registrationid as aregistrationid ,add.createdby as acreatedby, add.lastmodifiedby as alastmodifiedby, add.createdtime as acreatedtime, add.lastmodifiedtime as alastmodifiedtime";
    private static final String MAP_ADDRESS_SELECT_QUERY = " ,addmap.id as amid ,addmap.applicationnumber as amapplicationnumber ,addmap.tenantid as amtenantid";
    private static final String FROM_TABLES = " FROM eg_bt_registration btr LEFT JOIN eg_btr_address_map  addmap ON btr.applicationnumber  = addMap.applicationnumber LEFT JOIN eg_bt_address add  on addmap.id=add.registrationid ";
    ...
    public String getBirthApplicationSearchQuery(BirthApplicationSearchCriteria criteria, List<Object> preparedStmtList) {
        ...
        query.append(MAP_ADDRESS_SELECT_QUERY);
